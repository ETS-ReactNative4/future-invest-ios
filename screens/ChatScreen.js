...
const ChatScreen = () => {
  const {
    ...
    useEffect(() => {
      ...
      var headers = {};
      headers["Authorization"] = `Bearer ${user.memberTokenInfo.accessToken}`;
      ws.current = new WebSocket(WS_SERVER_URL, [],headers )
      ...
      ws.current.onclose = () => {
        console.log("[test ::: ] connection establish closed");
      }
      return () => {
        ws.current.close();
      };
